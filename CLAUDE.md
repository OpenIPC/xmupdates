...
- Asset names are `id{id}__{safe_version}__{obs_filename}` (catalog) or `p{id}__…` (portal), so different versions never collide on the release. The upload uses `--clobber`, so the script first checks the release's asset digests: an identical binary under the same name is reused, and a different one gets `__{sha256[:8]}` inserted before the obs filename.
